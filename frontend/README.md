...
> **Spec:** see [`../SYSTEM_DESIGN.md`](../SYSTEM_DESIGN.md) §4.1 (stack) and §8
> (routes, state, styling, a11y rules). This README is just the directory
> orientation; behavioural rules live in the spec.

React 19 + Vite 7 + TypeScript 5 (strict) SPA. TailwindCSS 4 is the only
styling system; auth uses Cognito Hosted UI via `react-oidc-context`.

> **Brief deviation:** the upstream Citi workshop brief mandated MUI;
> `SYSTEM_DESIGN.md` §1 records the v1.1 decision to go Tailwind-only. Every
> behaviour-bearing widget (dialog, menu, tabset, combobox) is therefore
> hand-rolled — the a11y rules below are non-negotiable.
...
src/
  ...
  pages/                  # one per route (see SYSTEM_DESIGN.md §8)
  services/
