...
> ## 📐 Start here: [`SYSTEM_DESIGN.md`](./SYSTEM_DESIGN.md)
>
> **`SYSTEM_DESIGN.md` is the canonical spec for this project** — stack, schema,
> API, RBAC, deployment, and the rules every agent and human must follow before
> touching code. Every other README in this repo is a thin, directory-local
> pointer back to it.
>
> This codebase was built around the upstream
> [**Citi `coding-workshop-participant`**](https://github.com/citi/coding-workshop-participant)
> brief (the original workshop scaffold + evaluation rubric still lives under
> [`docs/`](./docs/README.md)). `SYSTEM_DESIGN.md` is what that brief was bent
> into: a single concrete product (ACME Project Tracker) with locked-down
> architecture choices, a 3NF schema, and one Lambda per service. Where the
> brief and `SYSTEM_DESIGN.md` disagree, `SYSTEM_DESIGN.md` wins (see its §1
> "Brief deviation" row and §4.1 notes).
...
| [`docs/`](./docs/README.md)                           | Upstream Citi workshop brief + rubrics          | [`docs/README.md`](./docs/README.md)         |
...
password fields, and the "default password" footer on `/login`. The SPA falls
back to a plain email + password form that only accepts the roster accounts.
To override per-deploy, export `VITE_SEED_LOGIN_ENABLED` before running
`generate-env.sh`.
...
- **Project access is allocation-gated.** A non-owning `team_lead` or
  ...
  allocation (lands as `pending`); the owning lead approves it. Admins bypass.
...
Workshop docs (upstream Citi brief, evaluation rubric, testing guide):
[`docs/`](./docs/README.md). Architecture spec for **this** implementation:
