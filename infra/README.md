...
> **Spec:** the architecture this stack provisions is defined in
> [`../SYSTEM_DESIGN.md`](../SYSTEM_DESIGN.md) §4 (architecture) and §10
> (deployment). Resource decisions (no NAT, no API Gateway, Function URLs with
> `authorization_type=NONE`, etc.) are intentional — see the spec before
> "fixing" them.
...
- **Function URL `authorization_type = NONE`.** JWT validation is enforced
  ...
  in-handler check (see SYSTEM_DESIGN.md §9 + R-01).
...
- **Cognito is AWS-only.** LocalStack doesn't ship Cognito on the Community
  tier; the user pool is `count = 0` when
  `aws_caller_identity.id == "000000000000"`.
